"""Recover the saved four-route comparison plan without a new model call."""
...
try:
    ...
    from .plan_beta_coverage_cross_domain import persist_cross_domain_plan
except ImportError:
    from file_io import load_json, read_private_bytes, write_exclusive_json
    from model_call import MODEL, PROVIDER
    from plan_beta_coverage_cross_domain import persist_cross_domain_plan

from hermes_research_report.beta_coverage_query import build_cross_domain_query_prompt
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if (
            ...
            raise ValueError("coverage_cross_domain_reconciliation_not_allowed")
        ...
        if any(type(item) is not dict for item in (attempt, failure, usage, trace)):
            raise ValueError("coverage_cross_domain_saved_record_invalid")
        ...
        if (
            ...
            or not batch_run_id.endswith("-B03")
            or output.name != f"{batch_run_id}-coverage-source-plan"
        ):
            raise ValueError("coverage_cross_domain_saved_attempt_not_bound")
        run_id = batch_run_id.removesuffix("-B03")
        frame, _ = load_json(
        ...
        previous, _ = load_json(
            output.parent / f"{run_id}-B02-coverage-observed/batch-observation.json"
        )
        if any(
        ...
        if (
            attempt.get("purpose") != "coverage_cross_domain_query_plan"
        ...
        plan, batch = persist_cross_domain_plan(
            output=output,
            frame=frame,
            review=review,
            previous=previous,
        ...
        receipt = with_receipt_hash(
            {
                "schema_version": 1,
                "contract": "BetaCoverageCrossDomainReconciliation",
                "run_id": run_id,
        ...
        print(
            json.dumps(
                {
                    "status": "cross_domain_batch_reconciled",
                    "batch_run_id": batch_run_id,
