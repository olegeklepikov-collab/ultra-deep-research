"""One keyless OpenAlex Works metadata request from a verified beta plan."""
...
class AcquisitionDeadline(BaseException):
    """Abort the whole request, not merely one provider result."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if (
            ...
            or len(matching) != 1
            or matching[0]["source_family"] != "scholarly_index"
        ):
            raise AcquisitionError("openalex_leaf_not_routed")
        if args.output.name != f"{plan['run_id']}-{args.leaf_id}-openalex":
        ...
        try:
            receipt, artifacts = acquire_openalex_metadata(
